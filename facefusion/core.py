...
def run(program : ArgumentParser = None) -> None:
	...
	if facefusion.globals.force_download:
		force_download()
		return
	if not pre_check() or (facefusion.globals.ignore_content_analyser is not True and not content_analyser.pre_check()) or not face_analyser.pre_check() or not face_masker.pre_check() or not voice_extractor.pre_check():
		return
